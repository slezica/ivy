 ...
 * (pushed via adb by `bin/ivy.ts prepare --screenshots`), the database is wiped and
...
export const createSeedDemoData = (deps: SeedDemoDataDeps): SeedDemoData => (
  async () => {
    ...
    const settings = { sync_enabled: false, transcription_enabled: false, delete_original_after_import: false, clip_editor_linked: true }
