...
**Vendored shared libs.** The `:ffmpeg` package's libs link against four sonames it doesn't ship (`libexpat.so.1`, `libcrypto.so.3`, `libandroid-support.so`, `libandroid-posix-semaphore.so`) — upstream, those come from the yt-dlp python package we removed, so `modules/ivy` vendors them in its `jniLibs/` (extracted from the upstream `library-0.18.1` aar's `libpython.zip.so`, per ABI). `FFmpegEnvironment.ldLibraryPath()` builds the `LD_LIBRARY_PATH` for every `libffmpeg.so` exec and symlinks the two versioned sonames (which can't be jniLib filenames) into `noBackupFilesDir/ivy-native/lib`. When upgrading `youtubedl-android`, re-verify the dependency closure: every `NEEDED` soname reachable from `libffmpeg.so` must resolve from the package itself, the vendored libs, or the app's `nativeLibraryDir`. `scripts/check-ffmpeg-closure.js` checks this and cross-references `FFmpegEnvironment.SYMLINKED_LIBS` (so a vendored versioned lib present but not symlinked at runtime fails too); it runs automatically as a build finalizer of release/preview builds (`plugins/withIvyFfmpegClosureCheck.js`), and by hand as `node scripts/check-ffmpeg-closure.js <built-apk>`.
...
**Linked bounds ("bounds follow playhead").** The editor has a link toggle (lower right; its state is a remembered preference — `settings.clip_editor_linked`, default on). While linked, the playhead is solid: any playhead movement — scrub, fling, tap-to-seek, or playback itself — pushes the selection anchors it collides with, keeping the minimum clip duration between them (handles collide, then travel together). The core workflow: scrub to the start point, hit play, pause where the clip should end. Toggle the link off for free playhead movement. Design and rationale in [2026-07-24-linked-clip-bounds.md](2026-07-24-linked-clip-bounds.md); the push mechanic lives in the timeline physics engine (`timeline/engine.ts`, `linked` mode).
