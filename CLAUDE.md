...
- **Recovery only** — if `/workspace` was polluted anyway (a Gradle run in the mount, from either side; symptom above), fix it with `npm run clean` (deletes `local.properties`, `android/{build,app/build,.gradle}`, `modules/ivy/android/build`, and `node_modules/*/android/{build,.cxx}`). The next build on the affected machine is a slow full rebuild — that's why the isolation script is the rule, not cleaning. A safe alternative on the Mac: `npx expo prebuild --clean --platform android`.
